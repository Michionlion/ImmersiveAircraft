...
public class VehicleScreen extends AbstractContainerScreen<VehicleScreenHandler> {
    ...
    private void blitGui(GuiGraphics context, int x, int y, int u, int v, int w, int h, int textureWidth, int textureHeight) {
        context.blit(RenderPipelines.GUI_TEXTURED, TEXTURE, x, y, u, v, w, h, textureWidth, textureHeight);
    }

    protected void drawRectangle(GuiGraphics context, int x, int y, int h, int w) {
        //corners
        blitGui(context, x, y, 176, 0, 16, 16, 512, 256);
        blitGui(context, x + w - 16, y, 176 + 32, 0, 16, 16, 512, 256);
        blitGui(context, x + w - 16, y + h - 16, 176 + 32, 32, 16, 16, 512, 256);
        blitGui(context, x, y + h - 16, 176, 32, 16, 16, 512, 256);

        //edges
        blitGui(context, x + 16, y, 176 + 16, 0, w - 32, 16, 512, 256);
        blitGui(context, x + 16, y + h - 16, 176 + 16, 32, w - 32, 16, 512, 256);
        blitGui(context, x, y + 16, 176, 16, 16, h - 32, 512, 256);
        blitGui(context, x + w - 16, y + 16, 176 + 32, 16, 16, h - 32, 512, 256);

        //center
        blitGui(context, x + 16, y + 16, 176 + 16, 16, w - 32, h - 32, 512, 256);
    }

    public void drawImage(GuiGraphics context, int x, int y, int u, int v, int w, int h) {
        blitGui(context, x, y, u, v, w, h, 512, 256);
    }

    @Override
    protected void renderBg(@NotNull GuiGraphics context, float delta, int mouseX, int mouseY) {
        blitGui(context, leftPos, topPos, 0, 0, imageWidth, containerSize + TITLE_HEIGHT * 2, 512, 256);
        blitGui(context, leftPos, topPos + containerSize + TITLE_HEIGHT * 2 - 4, 0, 222 - BASE_HEIGHT, imageWidth, BASE_HEIGHT, 512, 256);
